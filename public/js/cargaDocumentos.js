document.getElementById('documentUploadForm').addEventListener('submit', async function (event) {
    ...
    formData.append('comprobanteDomicilio', document.getElementById('comprobanteDomicilio').files[0]);
    formData.append('comprobanteCuenta', document.getElementById('comprobanteCuenta').files[0]);
    const emailUsuario = localStorage.getItem('emailUsuario');
    
    try {
        const response = await fetch(`/api/usuarios/${emailUsuario}/documents`, {
            method: 'POST',
            body: formData
        });
        if (response.ok) {  
